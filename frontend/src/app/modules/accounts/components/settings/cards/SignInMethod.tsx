...
const SignInMethod: React.FC <detailsProps> = ({ accessToken }) => {
  ...
  const enableTwoFactorAuthentication = async (accessToken: any) => {
    try {
      const response = await axios.post('http://backend.tokenopp.org/api/auth/enableFA', null, {
        headers: {
      ...
      // Assuming the API returns the updated user data in response.data
...
// Inside the SignInMethod component
const formik1 = useFormik<IUpdateEmail>({
  ...
  onSubmit: async (values) => {
    setLoading1(true);
    try {
      const response = await axios.post('http://backend.tokenopp.org/api/auth/changeemail', values,{
        headers: {
          'Content-Type': 'application/json',
      ...
      setEmailUpdateData(response.data);
      setLoading1(false);
...
const formik2 = useFormik<IUpdatePassword>({
  ...
  onSubmit: async (values) => {
    setLoading2(true);
    try {
      const response = await axios.post('http://backend.tokenopp.org/api/auth/changepassword',values, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${accessToken}`, // Replace with the actual access token from your frontend authentication.
        },
      ...
      // Assuming the API returns the updated user data
      setPasswordUpdateData(response.data);
      setLoading2(false);
        ...
        <div className='card-body border-top p-9'>
          <div className='d-flex flex-wrap align-items-center'>
            <div id='kt_signin_email' className={' ' + (showEmailForm && 'd-none')}>
              ...
              <div className='fw-bold text-gray-600'>Example@email</div>
            </div>
            ...
            >
              ...
              >
                <div className='row mb-1'>
                  ...
                  <div className='col-lg-4'>
                    <div className='fv-row mb-0'>
                      <label htmlFor='confirmnewpassword' className='form-label fs-6 fw-bolder mb-3'>
                      ...
                      />
                      {formik2.touched.confirmnewpassword && formik2.errors.confirmnewpassword && (
                        <div className='fv-plugins-message-container'>
                          <div className='fv-help-block'>{formik2.errors.confirmnewpassword}</div>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className='notice d-flex bg-light-primary rounded border-primary border border-dashed p-6'>
            ...
            <div className='d-flex flex-stack flex-grow-1 flex-wrap flex-md-nowrap'>
              ...
              <button
                  onClick={enableTwoFactorAuthentication} // Call the function to enable 2FA
...
export default connect(mapStateToProps)( SignInMethod);
// export {SignInMethod}
